...
public class _00_SnakeGame implements ActionListener, KeyListener {
	...
	public void startGame() {
		...
		String instructions = "Move the snake using the arrow keys. The more it eats the dots the bigger it gets. Don't let the snake trip over himself...";
	...
	private void setFoodLocation() {
		...
		Location foodPlace = new Location(new Random().nextInt(WIDTH), new Random().nextInt(HEIGHT));
		...
		// on the snake
		if (!snake.isLocationOnSnake(foodPlace)) {
			foodLocation = foodPlace;
		}
	}
	...
	private void gameOver() {
		...
		String awnser = JOptionPane.showInputDialog(null, "Want To Play Again?");
		// 4. if they want to play again
		// reset the snake and the food and start the timer
		if(awnser.toLowerCase().startsWith("y")) {
